...
export interface ValidateOptions {
  /**
   * true면 밝기 체크 건너뜀.
   * AI 생성 이미지는 심볼이 샘플링 포인트를 가릴 수 있어 false positive 발생 → 생성 경로에서 사용.
   * 템플릿 경로는 항상 밝기 체크 수행 (흰 배경 방지).
   */
  skipBrightnessCheck?: boolean;
}

/**
 * 앱인토스 로고 가이드 검증:
 * - 600x600 PNG + 알파 채널
 * - 배경 밝기 HSL L ≤ 85% (흰색/미색 금지, 템플릿 경로만)
 * - 원형 아이콘: 모서리 픽셀 투명
 */
export async function validateLogo(
  ...
  if (meta.format === 'png' && meta.hasAlpha) {
    ...
    // 4. Background brightness (템플릿 경로만 — AI 생성 이미지는 skip)
    if (!opts.skipBrightnessCheck) {
